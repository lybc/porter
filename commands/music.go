...
func downloadSingle(ctx *cli.Context) error {
	id := getIdByUrl(ctx.Args().First())
	// 根据歌曲ID获取歌曲详情，如果无需打印歌曲信息可省略

	api := api.Netease{}
	ids := []string{id}
	songsDetail := api.GetSongDetail(ids, false)
	if songsDetail.Code != 200 {
		return fmt.Errorf("获取歌曲详情失败")
	}
	downloader := utils.NewDownloader(ctx.String("output"))
	fmt.Println(downloader)
	for _, song := range songsDetail.Songs {
		name := fmt.Sprintf("%s.mp3", song.Name)
		mp3Url := fmt.Sprintf(MusicMediaUrl, strconv.Itoa(song.ID))
		fmt.Println(mp3Url)
		downloader.AppendResource(name, mp3Url)
	}
